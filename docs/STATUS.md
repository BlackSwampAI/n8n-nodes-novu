...
- Status: implemented locally on `release/0.1.1-preparation`; human review and release authorization pending
...
- Authentication: the publish workflow is prepared for OIDC-only npm Trusted Publishing. The GitHub `NPM_TOKEN` secret was independently verified absent on September 9, 2026. Local npm is unauthenticated and could not query npm's Trusted Publisher configuration, so tagging/publication remains blocked until the owner confirms exact owner `BlackSwampAI`, repository `n8n-nodes-novu`, workflow `publish.yml`, no environment, and direct npm publish allowed.
...
- Human-only pending evidence: confirm npm Trusted Publisher, confirm revocation of the temporary 0.1.0 token, authorize the immutable 0.1.1 tag, observe PR/default-branch/tag/publish/provenance/published-package checks, create the GitHub release, and submit/inspect the exact Creator Portal version. n8n verification and complete live coverage are not claimed.
