...
Observed live coverage remains limited: the owner retrieved available objects and created a topic in an actual editor against Novu Cloud, but exact versions and operation-by-operation evidence were not recorded. Release remains subject to human authorization, Trusted Publisher confirmation, immutable tag, GitHub Actions publication, npm/provenance verification, and Creator Portal gates documented in `RELEASING.md`.
